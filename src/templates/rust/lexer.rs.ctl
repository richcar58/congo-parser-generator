...
impl Lexer {
    ...
    pub fn next_token(&mut self) -> ParseResult<Token> {
        ...
        [#-- This is a simplified version - full NFA-based matching would go here --]
    ...
    fn try_match_token(&mut self, start_pos: usize) -> ParseResult<Option<Token>> {
        [#-- Generate token matching for each regular expression in the grammar --]
        [#list lexerData.regularExpressions as regexp][#if !regexp.private && regexp.literalString?? && regexp.literalString?length > 0][#if regexp.label?starts_with("_TOKEN_")]
        // Match literal
        if self.matches_string("${regexp.literalString?replace("\\", "\\\\")?replace("\"", "\\\"")?replace("\n", "\\n")?replace("\r", "\\r")?replace("\t", "\\t")}") {
            let image = "${regexp.literalString?replace("\\", "\\\\")?replace("\"", "\\\"")?replace("\n", "\\n")?replace("\r", "\\r")?replace("\t", "\\t")}".to_string();
...
[/#if][/#if][#if !regexp.private && regexp.label == "INTEGER"]
