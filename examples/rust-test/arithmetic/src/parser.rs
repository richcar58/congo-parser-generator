//! Parser implementation for arithmetic expressions.
//! Generated by CongoCC Parser Generator - manually enhanced with arena integration.

use crate::arena::{Arena, AstNode, NodeId, TokenId, AdditiveOp, MultiplicativeOp};
use crate::arena::{ExpressionNode, AdditiveExpressionNode, MultiplicativeExpressionNode, PrimaryNode};
use crate::error::{ParseError, ParseResult};
use crate::tokens::{Token, TokenType};
use crate::lexer::Lexer;

/// The parser for arithmetic expressions with arena-based AST allocation
pub struct Parser {
    /// The lexer providing tokens
    lexer: Lexer,
    /// Current token being examined
    current_token: Token,
    /// Lookahead tokens for multi-token lookahead
    lookahead: Vec<Token>,
    /// Arena for allocating AST nodes
    arena: Arena,
    /// Current token ID (for tracking begin/end tokens)
...
impl Parser {
    ...
    /// Grammar: Expression -> AdditiveExpression <EOF>
    ...
    fn parse_additive_expression(&mut self) -> ParseResult<NodeId> {
        ...
        // Parse first multiplicative expression
        let first_child = self.parse_multiplicative_expression()?;
        children.push(first_child);

        // Parse zero or more (("+" | "-") MultiplicativeExpression)
        ...
        // Create AdditiveExpression node
    ...
    /// Parse: MultiplicativeExpression -> Primary (("*" | "/") Primary)*
    fn parse_multiplicative_expression(&mut self) -> ParseResult<NodeId> {
        ...
        // Parse first primary
        let first_child = self.parse_primary()?;
        children.push(first_child);

        // Parse zero or more (("*" | "/") Primary)
        ...
        // Create MultiplicativeExpression node
    ...
    /// Parse: Primary -> <INTEGER> | "(" AdditiveExpression ")"
    fn parse_primary(&mut self) -> ParseResult<NodeId> {
        ...
        if self.current_token.token_type == TokenType::INTEGER {
            ...
            // Parse inner additive expression
